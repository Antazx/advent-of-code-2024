...
export function main({ map, initialPosition }: { map: Map; initialPosition: Position }) {
  let positions: Set<Position> = new Set();
  ...
  while (true) {
    ...
    if (currentChar === TILES.CHANGE_DIRECTION_CHAR) {
      ...
      if (currentChar !== TILES.ALREADY_VISITED_CHAR) {
        positions.add(currentPosition);
      }
      currentPosition = nextPosition;
    }
  }
  const positionCount = positions.size + 1;
  return positionCount;
}

function simulateGuard(map: Map, initialPosition: Position, obstruction: Position | null = null) {
  const simulatedMap = structuredClone(map);
  const visited: Set<Position> = new Set<Position>();
